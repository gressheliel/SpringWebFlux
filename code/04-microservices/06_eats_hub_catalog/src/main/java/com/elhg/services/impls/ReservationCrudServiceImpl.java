...
import com.elhg.validators.ReservationValidator;
...
public class ReservationCrudServiceImpl implements ReservationCrudService {
    ...
    public Mono<ReservationCollection> createReservation(ReservationCollection reservation) {
        reservation.setId(UUID.randomUUID());  //Genera un ID aleatorio cuando se crea una reservation
        ...
        return reservationValidator.applyValidations(reservation, validations)
                .then(restaurantRepository //Entonces ejecuta la siguiente operacion
    ...
    public Mono<ReservationCollection> updateReservation(UUID id, ReservationCollection reservation) {
        final var validations = List.of(
                reservationValidator.validateRestaurantNotClosed(),
                reservationValidator.validateAvailability(),
                reservationValidator.validateRestaurantIdBeforeUpdate()
        );
        ...
        return reservationRepository.findById(id)
                ...
                .flatMap(existingReservation -> {
                    ...
                    return reservationValidator.applyValidations(reservation, validations)
                            .thenReturn(existingReservation);  //Entonces Regresa el existingReservation
                }).flatMap(existingReservation -> {
